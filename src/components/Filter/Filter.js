...
import { getWeb3Ethereum } from "../../helpers/wallet"
...
export const Filter = (props) => {
  ...
  const { walletAddress, walletFamily, synchroTables, handleSynchroTables } = useContext(AppContext)
  ...
  const voteClicked = async (id) => {
    if (walletFamily === "tron") {
      NotificationManager.error(
        "Voting uses Ethereum. Please disconnect and sign in with MetaMask, Rabby, or Phantom (Ethereum)."
      )
      return
    }
    const eth = getWeb3Ethereum()
    if (!eth) {
      NotificationManager.error("No Ethereum wallet in this browser session.")
      return
    }
    ...
    "MCAP",
    {
      ...
      options: {
        customBodyRender: (value, tableMeta, updateValue) => {
          ...
          if (isNaN(val)) {
            return (
              <div className="change24hDiv" style={{ color: "#e4e4e7" }}>
                <span>{val}</span>
              </div>
            )
          } else if (val < 0) {
            val = 0 - val
            return (
              <div className="change24hDiv" style={{ color: "#eab308" }}>
                <span>{"- " + val + " %"}</span>
              </div>
            )
          } else {
            return (
              <div className="change24hDiv" style={{ color: "#22c55e" }}>
      ...
      overrides: {
        ...
        MUIDataTableBodyRow: {
          root: {
            ...
            // borderColor:"#669999",
            "&:hover": {
              backgroundColor: "rgba(143, 132, 201, 0.12) !important",
            },
            height: "30px",
          },
        },
        ...
        MUIDataTableFilter: {
          root: {
            backgroundColor: "rgba(143, 132, 201, 0.12)",
          },
        },
            ...
            <div className="diamondFilterDiv">
              {diamondSelected === true ? (
                <div
                  ...
                  style={{
                backgroundImage:
                  "linear-gradient(to right, rgba(143, 132, 201, 0.52), rgba(111, 146, 184, 0.45))",
              }}
